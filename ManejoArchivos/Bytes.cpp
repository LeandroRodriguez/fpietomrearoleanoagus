...
bool Bytes::agregar(const Bytes& bytes,offset offset) {
	...
	this->bytes.insert(offset,bytes.toString());//a partir de la posici�n offset, guardo bytes
	return true;
}
...
bool Bytes::agregar(const string &bytes,offset offset) {
	...
	this->bytes.insert(offset,bytes);//a partir de la posici�n offset, guardo bytes
	return true;
}
...
bool Bytes::agregar(void* dato, int tamanio, offset offset) {
	...
	return agregar(str,offset);//agrega a this->bytes el string str en la posici�n offset
}
